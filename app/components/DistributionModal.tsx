  ...
  const handleCompleteDistribution = async () => {
    ...
    try {
        ...
        await runTransaction(db, async (transaction) => {
            ...
            // Register a read on the session doc (required when updating in a transaction)
            await transaction.get(sessionRef);

            const inventoryCache = new Map<string, { ref: ReturnType<typeof doc>; item: InventoryItem }>();

            // Gather all inventory reads first (Firestore requires reads before writes)
            for (const ingredient of session.ingredientUsage) {
                if (ingredient.inventoryItemId && !inventoryCache.has(ingredient.inventoryItemId)) {
                    const inventoryRef = doc(db, "inventory", ingredient.inventoryItemId);
                    const inventorySnap = await transaction.get(inventoryRef);
                    if (inventorySnap.exists()) {
                        inventoryCache.set(ingredient.inventoryItemId, {
                            ref: inventoryRef,
                            item: inventorySnap.data() as InventoryItem,
                        });
                    }
                }
            }

            const updates: Array<{ productName: string; oldQty: number; newQty: number }> = [];
            ...
            for (const ingredient of session.ingredientUsage) {
                ...
                if (ingredient.inventoryItemId) {
                    const cached = inventoryCache.get(ingredient.inventoryItemId);
                    if (cached) {
                        const { ref: inventoryRef, item } = cached;
                        const deduction = Math.min(actualQuantityNeeded, item.quantity);

                        if (deduction > 0) {
                            const newQty = Math.max(0, item.quantity - deduction);
                            const newDistributed = (item.distributedQuantity || 0) + deduction;
                            ...
                            if (newQty <= 0) {
                                ...
                                // Update cache so subsequent ingredients referencing the same item use new quantities
                                cached.item = {
                                    ...item,
                                    quantity: newQty,
                                    distributedQuantity: newDistributed,
                ...
                const variance = actualQuantityNeeded - deduction;
                            ...
                            updatedIngredient.variancePercentage = ingredient.expectedQuantity > 0
